...
class Rutracker:
    ...
    def search(self, search_line:str = '', sort_by:str = 'number_of_seeds'):
        """Search the tracker

        IN: Search line
        IN: sort_by
        - number_of_seeds (default)
        - number_of_leeches
        - date_added
        ...
        raw = self._ask_tracker('search', search=search_line, sort_by=sort_by)
    ...
    def _parse_table(self, raw):
        ...
        try:
            ...
            seeds = [int(i['data-ts_text']) for i in soup.findAll('td', {'class': 'row4 nowrap'})]
    ...
    def _ask_tracker(
            ...
            page_no:int = 1,
            topic_id:str = '',
            sort_by:str = 'number_of_seeds'
    ):
        # Choose request type
        if mode == 'search':
            sort_by_num = sort_by_mapping[sort_by]
            url = self.tracker_path + f'forum/tracker.php?f%5B%5D=-1&o={sort_by_num}&s=2&tm=32&pn=&nm={search}'
